...
- **Journal** — free-form entries plus the daily prompts, grouped by month. The composer lets you tag a free-form entry to what it's about — Freeform, or one of your active goals — as buttons, not a dropdown. The search/filter area mirrors that split: a "Prompts" row (Retrospective, Future-self, Socratic, Values, Motive, From your data) for the auto-generated daily entries, and a "Goals" row (Freeform + each active goal) for your own notes. Clicking a "→ Goal" pill on any entry jumps straight to that goal's filter.
