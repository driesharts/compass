...
- **Today** — two free-text plan boxes: "What do you want to get done today?" (usually pre-filled with whatever you wrote in the tomorrow box the day before) and "What do you want to get done tomorrow?" (a fresh one each day) — a lightweight evening-planning loop, separate from habits/to-dos on purpose since those already have their own checkable tracking. Below that, a flat To-do's list (not grouped by value — there usually aren't enough of them at once to need it), a Habits section grouped by the value each habit serves with a rolling 7-day consistency view per habit (not a fragile streak — missing a day doesn't reset anything), and an optional one-line daily reflection prompt.
- **Journal** — free-form entries plus the daily prompts, grouped by month, with search and category filters (Retrospective, Future-self, Socratic, Values, Motive, From your data, Free-form). A free-form entry can optionally be tagged to a goal via a dropdown in the composer; tagged entries show a "→ Goal" pill you can click to filter the whole journal down to that goal's notes.
